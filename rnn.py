...
class RNN:
    ...
    def rnn_loss(self, x, y):
        ...
        for idx in xrange(len(y)):
            ...
            output = softmax(np.dot(self.V, h[-1]))
    ...
    def rnn_bproptt(self, x, y):
        ...
        output = softmax(np.dot(self.V,h))
        d_output = output
        ...
        for idx in xrange(len(y)-1,-1,-1):
            ...
            xc = np.hstack((self.U[:,x[t]],h[idx-1]))
            ...
            for t in xrange(idx,max(0, idx-self.bptt_max)-1,-1):
                ...
                b = activ[t-1]**2
                d_t = a * (1 - b)'''
        return (dWg, dWi, dWf, dWo, dbg, dbi, dbf, dbg, dbo, dU, dV)
    ...
    def rnn_sgd(self, X, y, epochs=20, alpha=0.01, loss_epoch=5):
        ...
        for t in xrange(epochs):
            ...
            for i in xrange(len(y)):
                dWg, dWi, dWf, dWo, dbg, dbi, dbf, dbg, dbo, dU, dV = self.rnn_bproptt(X[i],y[i])
                self.Wg -= alpha*dWg
                self.Wi -= alpha*dWi
                self.Wf -= alpha*dWf
                self.Wo -= alpha*dWo
